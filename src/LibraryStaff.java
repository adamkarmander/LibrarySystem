	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (command == Command.LIST) {
				// Prints out article numbers, titles and inventory status for each registered
				// product.
				// Prints out the names and phone numbers of customers that have borrowed a
				// product.
			} else if (command == Command.CHECKOUT) {
				// If someone has borrowed the product
				System.out.println(
						"Cannot lend (product name) to another customer. It is already borrowed by (customer name)");
				// else
				...
				System.out.println(
						"(product type) (product name): Value (product value)kr, Number of pages: (book pages), Publisher (book author)");
				// else if it's a registered Movie
				System.out.println(
						"(product type) (product name): Value (product value)kr, Length (movie length), Rating (movie rating)");
				// else
